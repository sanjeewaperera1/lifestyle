...
if ($request->server['REQUEST_METHOD'] == 'POST' && $action_type == 'SEND') 
{
	try {
		...
		$data = array(
			...
			'payable_amount' => 'Amount: '.get_currency_code().' '.currency_format($invoice['payable_amount']).', ',
			'paid_amount' => 'Paid: '.get_currency_code().' '.currency_format($invoice['paid_amount']).', ',
			'due' => 'Balance: '. get_currency_code().' '.currency_format($invoice['due']),
			...
			'date' => 'Date: '.format_only_date($invoice['created_at']).', ',
		...
		if($message== 'SMS'){
    		...
    		$message = '[date] [invoice_id] [payable_amount] [paid_amount] [due]';
